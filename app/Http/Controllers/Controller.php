...
class Controller extends BaseController
{
    ...
    protected function storeImage($file, $folder, $isUpdate = false, $quality = 75, $width = 1024) {
        if (!Storage::exists('public/' . $folder)) {
            Storage::makeDirectory('public/' . $folder);
        }
        $tempPath = $file->store('temp');
        $extension = last(explode('.', $tempPath));
        $fileName = explode('.', last(explode('/', $tempPath)))[0] . '.' . $extension;
        $this->compressAndStoreImage(storage_path('app/' . $tempPath), storage_path('app/public/' . $folder . '/' . $fileName), $quality, $isUpdate, $width);
    ...
    protected function compressAndStoreImage($filePath, $destination, $quality = 75, $isUpdate = false, $width = 1024) {
